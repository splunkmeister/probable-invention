<#
.SYNOPSIS
  Build and configure the domain GPO "CIS - Windows Server 2025 - Domain Controllers - Level 1"
  (CIS Microsoft Windows Server 2025 Benchmark v2.0.0 - Level 1, Domain Controllers).
.DESCRIPTION
...
.EXAMPLE
  .\Create-CIS-DC-GPO.ps1 -WhatIf                                  # preview, no changes
.EXAMPLE
  .\Create-CIS-DC-GPO.ps1                                          # build + configure, do NOT link
.EXAMPLE
  .\Create-CIS-DC-GPO.ps1 -TargetOU "OU=Domain Controllers,DC=company,DC=com"                       # build + configure + link (goes live)
#>
...
$GpoName  = "CIS - Windows Server 2025 - Domain Controllers - Level 1"
$Scope    = "DC"                       # 'Member' or 'DC'
$InfFile  = Join-Path $ScriptRoot "CIS_Server2025_DC_Level1.inf"
$AuditCsv = Join-Path $ScriptRoot "CIS_AuditPolicy_DC.csv"
...
# USER Configuration, but this GPO is linked to a DC/computer OU, where user policy does not apply
# to the computer. So section 19 is NOT force-applied here - by design. They are low-value
# interactive-desktop controls, and forcing them would require User Group Policy loopback
# processing, which is commonly avoided on domain controllers. A CIS-CAT scan in an interactive
# session will report the six section-19 items as FAIL; that is expected. To enforce them, enable
# loopback (Merge) on this GPO or link a user-targeted GPO to the users' OU. See ExceptionsAndManualSteps.md 5.1.
Write-Host "[i] Section 19 (user-scope) settings are in the GPO but NOT force-applied on a DC OU (by design - see ExceptionsAndManualSteps.md 5.1)." -ForegroundColor Yellow
